...
def get_existing_runs(project_code, task_code):
    """Fetches existing runs for a given project and task code from the database."""
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute("""SELECT * FROM Define_RUN_NUM_Parameters
                      WHERE PROJECT_CODE = %s AND TASK_CODE = %s
                      AND RUN_NUMBER IS NOT NULL
                      ORDER BY id DESC""", (project_code, task_code))
...
def assign_run_number(project_code, task_code, job_description, workflow_steps, iterator):
    ...
    previous_entries = get_existing_runs(project_code, task_code)
    run_number = None

    workflow_steps = workflow_steps.upper()

    if workflow_steps in ['PRE', 'PRE RUN', 'ALL']:
        run_number = generate_run_number(project_code, task_code)
    elif workflow_steps == 'RUN':
        jd_match = [entry for entry in previous_entries if entry[4] == job_description]
        if jd_match:
            pre_exists = any('PRE' in entry[7].upper() for entry in jd_match)
            run_exists = any('RUN' in entry[7].upper() for entry in jd_match)
            iterator_has_value = any(entry[9] for entry in jd_match)

            if pre_exists:
                run_number = jd_match[0][3]
            elif run_exists and iterator_has_value:
                run_number = jd_match[0][3]
            else:
                run_number = generate_run_number(project_code, task_code)
        else:
            run_number = generate_run_number(project_code, task_code)
    elif workflow_steps == 'RUN POST':
        jd_match = [entry for entry in previous_entries if entry[4] == job_description]
        if jd_match:
            pre_exists = any('PRE' in entry[7].upper() for entry in jd_match)
            run_exists = any('RUN' in entry[7].upper() for entry in jd_match)
    ...
    return run_number if run_number else generate_run_number(project_code, task_code)
...
class WorkflowArgs:
	...
	def set_members(self, dict : dict):
		...
		self.run_number     = dict.get("RUN_NUMBER") # This will now be automated
	...
	def run_number(self, value):
		if value and value.lower() == 'auto':
			self._run_number = assign_run_number(
				self.project_code,
				self.task_code,
				self.description,
			...
			simple_exit("RUN_NUMBER is a mandatory parameter. Set to 'auto' for automatic numbering.")
