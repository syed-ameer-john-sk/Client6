...
def process_job_data(job_folder_path):
    ...
    for file_path in simulation_files_cfx:
        try:
            ...
            for index, row in df.iterrows():
                cursor.execute(f"""INSERT INTO {db_table.Ext_Aero_CFx}
    ...
    for file_path in simulation_files_cfz:
        try:
            ...
            for index, row in df.iterrows():
                cursor.execute(f"""INSERT INTO {db_table.Ext_Aero_CFz}
    ...
    def read_param(base_dir, target_file = 'parameters.txt'):
        keywords = {'DESCRIPTION:': None, 'SOLVER_VERSION:': None, 'QUEUE:': None, 'TEMPLATE:': None}
        for root, dirs, files in os.walk(base_dir):
    ...
    if params:
        try:
            mapped_params = {'JOB_DESCRIPTION': params['DESCRIPTION:'], 'SOLVER_VERSION': params['SOLVER_VERSION:'], 'QUEUE_TYPE': params['QUEUE:'], 'TEMPLATE': params['TEMPLATE:']}
            df = pd.DataFrame([mapped_params], columns=['JOB_DESCRIPTION', 'SOLVER_VERSION', 'QUEUE_TYPE', 'TEMPLATE'])
            for index, row in df.iterrows():
    ...
    try:
        if run_code: # Only insert if we have a valid run_code
            cursor.execute(f"""INSERT INTO {db_table.table_elapse} (RUN_CODE, PROJECT_CODE, TASK_CODE, RUN_NUMBER, SUBMISSION_DATE, USER_NAME, 
                            PRE_JOB_ID, PRE_ELAPSED_TIME,
                            RUN_JOB_ID, RUN_ELAPSED_TIME, 
                            POST_JOB_ID, POST_ELAPSED_TIME) 
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""", 
                            (run_code, project_code, task_code, run_num, actual_date, user_name, 
